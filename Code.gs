...
function analyze(){
  ...
  var studentCheck = sheet.getRange("D:D").getDisplayValues();
  var studentIndex = [];
  var studentNamesF = sheet.getRange("B:B").getDisplayValues();
  var studentNamesL = sheet.getRange("C:C").getDisplayValues();
  
  for(var j = 0; j<studentCheck.length; j++){
    if(studentCheck[j] == "TRUE"){
      studentIndex.push(j);
    }
  }
  for(var j = 0; j<studentIndex.length; j++){
    var studentRow = sheet.getRange(studentIndex[j]+":"+studentIndex[j]).getDisplayValues()[0];
    ...
    var studentSheet = ss.insertSheet(studentNamesF[studentIndex[j]]+" "+studentNamesL[studentIndex[j]]);
